...
if (platform == 'darwin') {
  trayIcon = path.join(__dirname, 'assets/coursera-logo.png')
} else if (platform == 'win32') {
  trayIcon = path.join(__dirname, 'assets/coursera-logo.ico')
} else {
  trayIcon = path.join(__dirname, 'assets/coursera-logo.ico')
}
...
function createWindow() {
  ...
  const url = 'https://coursera.org' // process.argv[2]
  ...
  const menuTemplate = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'Learn More',
          click: () => {require('electron').shell.openExternal('https://github.com/alexdevero/coursera-desktop-app')}
        },
        {
          label: 'Author',
  ...
  appIcon.setTitle('Coursera Desktop App')

  // Set toot tip for tray icon
  appIcon.setToolTip('Coursera Desktop App')
